...
A1L             equ $3C         ; move source start
A1H             equ $3D
A2L             equ $3E         ; move source end
A2H             equ $3F
A4L             equ $42         ; move dest start
A4H             equ $43
*
...
*
************ FP routines ************
fac             equ $9D         ; FAC : $9D to $A2 (6 bytes)
arg             equ $A5         ; ARG : $A5 to $AA
facsign         equ $A2
argsign         equ $AA
...
F2INT16         equ $E752       ; FAC to 16 bits int in A/Y and $50/51 (high/low)
FADD            equ $E7BE       ; Move the number pointed by Y,A into ARG and fall into 
FADDT           equ $E7C1        ; FAC = FAC + ARG if Z is O, else copy ARG into FAC (FAC=0+ARG)
FADDFAC         equ $E7C6       ; FAC = FAC + ARG 
FSUBT           equ $E7AA       ; FAC = ARG - FAC
FMULT           equ $E97F       ; Move the number pointed by Y,A into ARG and fall into FMULTT 
FMULTT          equ $E982       ; FAC = FAC x ARG (Z must be clear, else rts)
FDIVT           equ $EA69       ; FAC = ARG / FAC
...
; In executing the following subroutines, Applesoft expects the argument to be in the FAC. 
; After the result has been calculated, it will be placed in the FAC.
LOG             equ $E941 
ABS             equ $EBAF 
SQR             equ $EE8D 
;EXP             equ $EF09 
