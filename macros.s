...
DoCompare16     MAC               ; compare two 16 bit integer 
        lda ]1+1                  ; number 1 on stack
        pha
        lda ]1
        pha
        lda ]2+1                 ; number 2 on stack
        pha
        lda ]2
        pha  
        jsr ]3                  ; compare routine (works for signed or unsigned numbers)
        EOM

DoDivid16       MAC
        lda ]1+1                 ; dividend on stack
        pha
        lda ]1
        pha
        lda ]2+1                 ; divisor on stack
        pha
        lda ]2
        pha  
        jsr ]3                  ; call division routine
        EOM
...
addInt  MAC                     ; add two 16 bits unsigned; result in first arg.
        clc
        lda ]1
        adc ]2
        sta ]1
...
addInt32 MAC                     ; add two 32 bits unsigned; result in first arg.
        clc
        lda ]1
        adc ]2
        sta ]1
...
subInt32 MAC                     ; substract two 32 bits unsigned; result in first arg.
        sec
        lda ]1
        sbc ]2
        sta ]1
